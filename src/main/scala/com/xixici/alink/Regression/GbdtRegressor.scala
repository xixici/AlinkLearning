...
object GbdtRegressor {
  def main(args: Array[String]): Unit = {
    ...
    val gbdtRegressor = new GbdtRegressor()
      .setFeatureCols(
        "R_D_Spend",
        "Administration",
        "Marketing_Spend"
    ...
    val model = pipeline.add(gbdtRegressor).fit(trainData)
    ...
    model.save(f"model/GbdtRegressor-$rmse(RMSE).csv")
